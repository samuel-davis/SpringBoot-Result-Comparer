...
package com.davis.utilities.result.compare.confusion;

import com.davis.utilities.result.compare.api.Prediction;
...
public class ConfusionMatrix
{
  ...
  private Map<String,List<Prediction>> resultsMap = new HashMap<>();
  ...
  public void increaseValue(String goldValue, String observedValue, int times, String fileName)
  {
    if(!resultsMap.containsKey(fileName)){
      resultsMap.put(fileName, new ArrayList<>());
    }
    if (goldValue.equals(observedValue)) {
      resultsMap.get(fileName).add(new Prediction(observedValue, goldValue, fileName, true ));
    }else{
      resultsMap.get(fileName).add(new Prediction(observedValue, goldValue, fileName, true ));
    }
    increaseValue(goldValue, observedValue, times);
  }
  ...
  public List<List<String>> prepareToString()
  {
    // adding zeros
  ...
  public Map<String, List<Prediction>> getResultsMap() {
    return resultsMap;
  }

  public void setResultsMap(Map<String, List<Prediction>> resultsMap) {
  ...
  public static ConfusionMatrix createCumulativeMatrix(ConfusionMatrix... matrices)
  {
    ...
    for (ConfusionMatrix matrix : matrices) {
      ...
      for(Map.Entry<String, List<Prediction>> entry : matrix.getResultsMap().entrySet()){
        if(!result.getResultsMap().containsKey(entry.getKey())){
          result.getResultsMap().put(entry.getKey(), entry.getValue());
        }else{
          result.getResultsMap().get(entry.getKey()).addAll(entry.getValue());
        }
      }
    }
